...
<html>
    <head>
        <title>Limbo Admin | Found Items</title>
        <meta charset="UTF-8">
    ...
    <body onload="startTime()">
        <header>
            ...
            <nav>
                <div class="nav-wrapper">
                    <a id="mainPage" href="#" class="brand-logo center">Admin Found Items Page</a>
								...
								<?php
									...
									if($results) {
										while($row = mysqli_fetch_array($results , MYSQLI_ASSOC)) {
											echo '<li>' . '<a href="found.php?location=' . $row['id'] . '">' . $row['short_name'] . '</a>' . '</li>';
										}
									}
								?>
							</ul>
						...
						<a class='dropdown-button btn' href='#' data-activates='category_drop'><i class="material-icons right">keyboard_arrow_down</i>Category</a>
							<ul id='category_drop' class='dropdown-content'>
								<?php
									...
									if($results) {
										while($row = mysqli_fetch_array($results, MYSQLI_ASSOC)) {
											echo '<li>' . '<a href="found.php?category=' . $row['id'] . '">' . $row['name'] . '</a>' . '</li>';
										}
									}
								?>
							</ul>
